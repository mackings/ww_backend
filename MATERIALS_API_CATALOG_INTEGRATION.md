...
- `thicknessUnit` (string, optional, default `inches`; allowed aliases include `mm`, `cm`, `m`, `meters`, `inches`, `ft`, `feet`; `meters` stores as `m`, `feet` stores as `ft`)
...
- `pricingUnit` (string, optional; default inferred from unit; allowed values include `sqm`, `piece`, `yard`, `meter`, `pound`, `bag`, `liter`, `gallon`, `kilogram`, `pair`, `pack`, `roll`, `set`, `bucket`)
- `unit` (string, optional display unit; mobile can send `Piece`, `Yard`, `Liter`, `Pound weight`, `Gallon`, `Kilogram`)
- `standardWidth`, `standardLength`, `standardUnit` (optional; for sheet/sqm calculations; `standardUnit` accepts `meters` and `feet` aliases)
